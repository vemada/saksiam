<?php
    ...
    $strJsonFileContents = file_get_contents("../web/json/sys_org.json");
    $orgs = json_decode($strJsonFileContents, true);

    // $org_chucks = array_chunk($orgs['results'],100);
    // $org_nos = $org_chucks[1];

    $org_nos = $orgs['results'];

    // $org_nos = array_slice($org_nos, 0, 10);

    $gen_month = $_POST['gen_month'] ?? '2021-08-31';
    ...
    $year  = date("Y");
    $month_name = $month_arr[date("n", strtotime(date($gen_month)) )];

    $start_month = date('Y-m-01 0:0:0', strtotime($gen_month));
    $end_month   = date("Y-m-d 23:59:59", strtotime($gen_month));

    $month_1 = strtotime($start_month);
    $month_2 = strtotime($end_month);

    // create your zip file
    $zipname = 'rtp-040-'.$gen_month.'.zip';
    $zip = new ZipArchive;
    $zip->open($zipname, ZipArchive::CREATE);
    $headers = ["no", "รหัส", "ชื่อ-สกุล", "ค่างวดต่อเดือน", "งวดที่เก็บ", "จำนวนเงิน", "ยอดค้าง", "เป้าหมาย", "ยอดเก็บ", "ค้างใหม่", "ล่วงหน้า", "ปรับ", "ปิดสด", "รถยึด", "เงินต้นชำระ", "ดอกเบี้ย", "ค่าธรรมเนียมบริการ", "ประเภทสินเชื่อ", "สถานะลูกค้า", "สถานะสัญญา", "Type"];

    foreach( $org_nos as $org ){
        ...
        $result = array('no'=>'Total','code' => '' ,'name'=>'','a'=>'','b'=>'' ,'c'=>'','d'=>'',
        ...
        $filename = 'rtp-040_'. $org_no . '_' .$org_name.' ประจำเดือน '.$month_name.' '.$year;
        
        // $fileDir  = '../reports/rtp-040/'.$gen_month;
        ...
        $zip->addFromString($filename.$fileExt, stream_get_contents($fd) );
        //close the file
        fclose($fd);
